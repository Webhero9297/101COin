...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 101 at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for z101 send
enum ZerocoinSpendStatus {
    Z101_SPEND_OKAY = 0,                            // No error
    Z101_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    Z101_WALLET_LOCKED = 2,                         // Wallet was locked
    Z101_COMMIT_FAILED = 3,                         // Commit failed, reset status
    Z101_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    Z101_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    Z101_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    Z101_TRX_CREATE = 7,                            // Everything related to create the transaction
    Z101_TRX_CHANGE = 8,                            // Everything related to transaction change
    Z101_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    Z101_INVALID_COIN = 10,                         // Selected mint coin is not valid
    Z101_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    Z101_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    Z101_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    Z101_SPENT_USED_Z101 = 14,                      // Coin has already been spend
    Z101_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    Z101_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void Z101BackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZ101OutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fz101Auto) const;
    ...
    */
    ...
    Cz101Wallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<Cz101Tracker> z101Tracker;
    ...
    void setZWallet(Cz101Wallet* zwallet)
    {
        zwalletMain = zwallet;
        z101Tracker = std::unique_ptr<Cz101Tracker>(new Cz101Tracker(strWalletFile));
    }

    Cz101Wallet* getZWallet() { return zwalletMain; }
    ...
    void setZ101AutoBackups(bool fEnabled)
    {
    ...
    /** z101 reset */
    boost::signals2::signal<void()> Notifyz101Reset;
