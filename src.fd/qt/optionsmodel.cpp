...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::101);
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in 101coin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case Anonymize101xAmount:
            return QVariant(nAnonymize101xAmount);
        case Listen:
