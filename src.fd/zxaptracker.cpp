...
Cz101Tracker::Cz101Tracker(std::string strWalletFile)
{
...
Cz101Tracker::~Cz101Tracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void Cz101Tracker::Init()
{
...
bool Cz101Tracker::Archive(CMintMeta& meta)
{
...
bool Cz101Tracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta Cz101Tracker::Get(const uint256 &hashSerial)
{
...
CMintMeta Cz101Tracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool Cz101Tracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> Cz101Tracker::GetSerialHashes()
{
...
CAmount Cz101Tracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount Cz101Tracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> Cz101Tracker::GetMints(bool fConfirmedOnly) const
{
...
bool Cz101Tracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool Cz101Tracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool Cz101Tracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool Cz101Tracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool Cz101Tracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool Cz101Tracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool Cz101Tracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void Cz101Tracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void Cz101Tracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void Cz101Tracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void Cz101Tracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void Cz101Tracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool Cz101Tracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> Cz101Tracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dz101 from DB\n", __func__, listDeterministicDB.size());
    }
