...
*Setup instructions for a gitian build of 101coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the 101coin
...
- [Building 101coin](#building-101coin)
...
Clone the git repositories for 101coin and gitian and then checkout the 101coin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/101coin-crypto/101coin.git
cd 101coin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the 101coin repository
...
To build 101coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the 101coin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/101coin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/101coin-crypto/101coin
...
For example:
```bash
URL=https://github.com/crowning-/101coin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit 101coin=${COMMIT} --url 101coin=${URL} ../101coin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 101coin=${COMMIT} --url 101coin=${URL} ../101coin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 101coin=${COMMIT} --url 101coin=${URL} ../101coin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/101coin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/101coin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/101coin-build.assert
```
...
[101coin/gitian.sigs](https://github.com/101coin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
