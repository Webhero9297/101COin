...
Cz101Wallet::Cz101Wallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing z101 seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZ101Seed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZ101Seed_deprecated()) {
                    LogPrintf("%s: Updated z101 seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old z101 seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of z101 wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool Cz101Wallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZ101Count(nCountLastUsed);
    else if (!walletdb.ReadZ101Count(nCountLastUsed))
        nCountLastUsed = 0;
...
void Cz101Wallet::Lock()
{
    seedMaster = 0;
}

void Cz101Wallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void Cz101Wallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZ101(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool Cz101Wallet::LoadMintPoolFromDB()
{
...
void Cz101Wallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void Cz101Wallet::GetState(int& nCount, int& nLastGenerated)
{
...
void Cz101Wallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->z101Tracker->HasPubcoinHash(pMint.first)) {
...
bool Cz101Wallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZ101(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to z101Tracker which also adds to database
    pwalletMain->z101Tracker->Add(dMint, true);
...
void Cz101Wallet::SeedToZ101(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 Cz101Wallet::GetZerocoinSeed(uint32_t n)
{
...
void Cz101Wallet::GenerateDeterministicZ101(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void Cz101Wallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool Cz101Wallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
